...
import type { MindMapData, CommandId, MindMapNodeData, NodeType, NodePriority, DataChangeCallback, CanvasTransform, ReviewStatusCode } from '../types';
...
import { ReviewContextMenu } from './ReviewStatusModal';
...
import { countAllDescendants, findAllDescendantUuids, findAllAncestorUuids, findAllDescendantUseCaseUuidsAndIds } from '../utils/findAllDescendantIds';
...
interface MindMapCanvasProps {
    ...
    onBulkUpdateReviewStatus: (nodeUuid: string, newStatus: ReviewStatusCode, useCaseUuids: string[], useCaseIds: (string | number)[]) => void;
    onSave: () => void;
    showAITag: boolean;
    isDraggable?: boolean;
    ...
    // FIX: Replaced enableUseCaseReview with more specific props to fix type error in App.tsx
...
export const MindMapCanvas: React.FC<MindMapCanvasProps> = ({
    mindMapData, onAddChildNode, onAddSiblingNode, onDeleteNode, onFinishEditing, onUpdateNodePosition, onReparentNode, onReorderNode, onLayout, onUpdateNodeSize, onSave, showAITag, isDraggable = false, enableStrictDrag = false, enableNodeReorder = true, reorderableNodeTypes, showNodeType, showPriority, onToggleCollapse, onExpandNodes, onExpandAllNodes, onCollapseAllNodes, onExpandToLevel, onCollapseToLevel, onUpdateNodeType, onUpdateNodePriority, onBulkUpdateReviewStatus,
    ...
    const [reviewContextMenu, setReviewContextMenu] = useState<ReviewContextMenuState>({ isVisible: false, nodeUuid: null, x: 0, y: 0 });
    ...
    const handleCloseReviewContextMenu = useCallback(() => {
        setReviewContextMenu(prevState => ({ ...prevState, isVisible: false }));
    }, []);

    const handleSelectReviewStatus = useCallback((newStatus: ReviewStatusCode) => {
        if (!reviewContextMenu.nodeUuid) return;

        const { uuids, ids } = findAllDescendantUseCaseUuidsAndIds(mindMapData, reviewContextMenu.nodeUuid);
        
        onBulkUpdateReviewStatus(reviewContextMenu.nodeUuid, newStatus, uuids, ids);
    }, [reviewContextMenu.nodeUuid, mindMapData, onBulkUpdateReviewStatus]);


    useEffect(() => {
        if (!contextMenu.isVisible && !canvasContextMenu.isVisible && !reviewContextMenu.isVisible) return;
        ...
        const handleClickOutside = (event: MouseEvent) => {
            ...
            if (contextMenu.isVisible || canvasContextMenu.isVisible || reviewContextMenu.isVisible) {
                 if (!target.closest('.context-menu') && !target.closest('.review-context-menu')) {
    ...
    }, [contextMenu.isVisible, canvasContextMenu.isVisible, reviewContextMenu.isVisible, handleCloseContextMenu, handleCloseCanvasContextMenu, handleCloseReviewContextMenu]);
        ...
        >
            ...
            {reviewContextMenu.isVisible && reviewContextMenuNode && (
                <ReviewContextMenu
                    ...
                    onSelectStatus={handleSelectReviewStatus}
                />
            )}
        </div>
    );
