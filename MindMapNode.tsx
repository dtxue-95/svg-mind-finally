...
const MindMapNodeComponent: React.FC<MindMapNodeProps> = ({
    ...
    const shouldShowReviewInfo = showReviewStatus || showRemarkIcon || showScoreInfo;
    ...
    const renderRemarkIcon = () => {
        return <FiMessageSquare className={`review-icon review-icon--remark ${node.hasRemark ? 'active' : ''}`} title={node.hasRemark ? '有备注' : '无备注'} />;
    };

    const renderScoreInfo = () => {
        return (
            <div className="review-score-container">
                <FiStar className={`review-icon review-icon--score ${node.hasScore ? 'active' : ''}`} />
                {node.hasScore && node.scoreInfo && <span className="review-score-value">{node.scoreInfo.scoreValue}</span>}
            </div>
        );
    };
