...
- **状态管理**:
    ...
    - **智能状态保持**: 在外部数据更新时（如保存后重新获取数据），能够保持画布的缩放/平移位置和节点的折叠状态，避免界面跳动。
- **高度可定制**:
...
| `preserveStateOnDataUpdate`| `boolean`                                  | **核心功能**。当 `initialData` prop 更新时，是否保留当前画布的视图（缩放/平移）和节点的折叠状态。为 `true` 时，组件会智能合并新旧数据，避免界面跳动。            | `true`                                                   |
| `stableNodeId`             | `string`                                   | 用于在 `preserveStateOnDataUpdate` 为 `true` 时匹配新旧节点的稳定 ID 字段名。应为 `RawNode` 对象中的一个唯一标识符（如数据库主键 `id`）。                  | `'id'`                                                   |
...
2.  **数据刷新 (重要)**: 当你需要用来自 API 的新数据刷新思维导图时：
    - 只需更新传递给 `initialData` prop 的 `mindMapData` 对象。
    - 组件将自动检测到 `initialData` 的变化。
    - 凭借 `preserveStateOnDataUpdate={true}` (默认) 和 `stableNodeId='id'` (默认)，组件会：
        a.  遍历新的数据树。
        b.  使用每个节点上的 `id` 字段来查找屏幕上已存在的对应节点。
        c.  如果找到匹配项，它将保留该节点当前的位置、尺寸和折叠状态，同时更新其他所有数据（如 `name`, 状态等）。
        d.  重新进行自动布局，这只会影响新添加的节点，而不会移动现有节点。
        e.  **用户的缩放和平移位置将保持不变。**

这个流程确保了无论是保存后还是执行用例后，数据刷新对用户来说都是无缝的。
